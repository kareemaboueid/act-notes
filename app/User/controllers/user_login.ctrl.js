...
import utl_sign_jwt from '../../../utilities/sign_jwt.utl.js';
import { JWT_NAME, NODE_ENV } from '../../../configs/env/env.cnfg.js';
...
const ctrl_user_login = asyncHandler(async (_request, _response) => {
  try {
    ...
    const is_password_valid = await DB_USER.validate_password(eud_user_password);
    ...
    // signed token:
    const signed_token = utl_sign_jwt({ username: eud_user_username }, '30d');
