...
import { JWT_SECRET } from '../configs/env.cnfg.js';

/**
 * ### sign_jwt: Sign JWT data.
 * @param {object} _payload - Payload data to sign.
 * @param {string} _expiresIn - Expiry time for the JWT token.
 * @returns {string} Signed JWT token.
 * @example
 * ```javascript
 * const signed_jwt_token = sign_jwt({ username: 'JohnDoe' }, '30d');
 * ```
 */
const sign_jwt = (_payload, _expiresIn) => {
  // ------ START ------ //
  ...
  if (typeof _expiresIn !== 'string')
    throw new Error('ExpiresIn must be a string');

  // signed JWT token
  const sign_jwt_token = jwt.sign(_payload, JWT_SECRET, {
